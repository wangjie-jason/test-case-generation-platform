"""token 用量采集的测试（app/services/usage_service.py）。

只测纯逻辑（contextvars 的采集/隔离、usage 解析、周起始），不碰数据库——
flush/summary 需要 async session，留给手工验证与实际运行覆盖。
...
def test_无收集器时静默跳过不抛异常():
    ...
    usage_service.record("m", {"total_tokens": 100})


def test_采集单条并带上阶段标注():
    with usage_service.collector() as records:
        with usage_service.stage(usage_service.STAGE_REVIEW):
            usage_service.record("deepseek-v4-pro", {
...
def test_缺_total_时用_prompt_加_completion_兜底():
    with usage_service.collector() as records:
        usage_service.record("m", {"prompt_tokens": 100, "completion_tokens": 50})
...
def test_全零或非法_usage_一律丢弃(usage):
    with usage_service.collector() as records:
        usage_service.record("m", usage)
    assert records == []


def test_非_dict_的_usage_不入库():
    with usage_service.collector() as records:
        usage_service.record("m", None)
        usage_service.record("m", "oops")
    assert records == []
    ...
    with usage_service.collector() as records:
        with usage_service.stage(usage_service.STAGE_GENERATE):
            with usage_service.stage(usage_service.STAGE_SUPPLEMENT):
                usage_service.record("m", {"total_tokens": 1})
            # 内层退出后应回到 generate，而不是残留 supplement
            usage_service.record("m", {"total_tokens": 2})
...
def test_收集器退出后不再采集():
    ...
    usage_service.record("m", {"total_tokens": 999})
    ...
    async def main() -> list[dict]:
        with usage_service.collector() as records:
            await asyncio.gather(
                worker(usage_service.STAGE_GENERATE, 0.02),
                worker(usage_service.STAGE_REVIEW, 0.01),
                worker(usage_service.STAGE_SUPPLEMENT, 0.0),
            )
            return list(records)
...
def test_week_start_取周一零点(now, expected):
    assert usage_service.week_start(now) == expected


def test_所有阶段常量都有中文标签():
    """新增阶段时若忘了补 STAGE_LABELS，看板会露出裸的英文 stage 名。"""
    stages = [
        usage_service.STAGE_CLARIFY, usage_service.STAGE_MODULE_SPLIT,
        usage_service.STAGE_GENERATE, usage_service.STAGE_REVIEW,
        usage_service.STAGE_SUPPLEMENT,
    ]
    for s in stages:
        assert s in usage_service.STAGE_LABELS
