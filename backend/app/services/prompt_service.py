...
- **仅当预期有两条及以上时才编号**；只有一条预期就直接写结论，不要带步骤号
  （对：`提示"密码修改成功"`；错：`4. 提示"密码修改成功"`——编号此时无对应关系可表达）
- 示例：
...
```"""
    ...
    def build_module_split(requirement_text: str, prd_chunks: list[dict] | None = None) -> tuple[str, str]:
        ...
        「命名一致」与「同类相邻」两条约束是实测补上的：模块名会被生成阶段沿用为用例
        标题的层级路径（见 build() 的 module_focus 子句），所以拆分阶段的用词与顺序会
        一路影响到评审分组、补充任务分组与最终列表顺序。曾出现同一平台被叫成「移动」和
        「移动端」两种写法（468 条用例分属两个顶层名），以及 PC 类模块被排成 1-6 与
        15-17 两段、导致最终列表里 PC 出现在两处。
        """
        prd_text = ""
        if prd_chunks:
            unique_texts = list(dict.fromkeys(d.get("text", "") for d in prd_chunks if d.get("text")))
...
3. **命名一致**：若多个模块同属一个更大的领域（同一个端/平台/子系统等），该领域名必须在这些模块名里**逐字相同**，不能同义换词。
   - ❌ 反例：`移动端工作台` 与 `移动我的走访` 混用了「移动端」「移动」两种写法
   - ✅ 正例：`移动端工作台`、`移动端我的走访` 一律用「移动端」
   - 反之，**需求本身只有一个领域时不要硬加公共前缀**：直接叫"工作台""我的任务"就好，不要造出"某系统-工作台"这类多余层级。用例标题的层级路径会沿用模块名，凭空多一级会让后续的分组与排序全部错位。
4. **同类相邻**：modules 数组的顺序要把同属一个领域的模块排在一起，不要来回交错。
   - ❌ 反例：`PC工作台, 移动端工作台, PC我的任务`（PC 被拆散到两处，最终用例列表里同一个端会出现在多段）
   - ✅ 正例：`PC工作台, PC我的任务, 移动端工作台`
5. 覆盖需求中**所有章节和功能领域**，不得遗漏。
6. output 严格为 JSON 对象（不要 markdown 代码块），含三个字段：
   - "modules": 有序的模块名列表（数组）
