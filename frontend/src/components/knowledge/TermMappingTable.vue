...
const columns = [
  { prop: 'ui_term', label: '页面术语' },
  // 纯展示列：页面术语 ↔ 技术字段之间的双向箭头
  { prop: 'arrow', label: '映射', width: 60, align: 'center' as const, display: true },
  { prop: 'tech_field', label: '技术字段' },
  { prop: 'mapping_desc', label: '映射说明' },
]
</script>

<template>
  <KnowledgeResourceTable
    :items="items" :columns="columns" :form-component="TermMappingForm"
    add-label="添加映射" dialog-label="映射"
